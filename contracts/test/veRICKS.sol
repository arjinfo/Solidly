...
contract RICKS is ERC20, ERC721Holder {
    ...
    /// @notice token for bidding auctions
    ...
    /// @notice the ERC721 token ID bing fractionalized
    uint256 public id;
    ...
    constructor(string memory _name
        ...
        stakingPool = address(new StakingPool(address(this), _token));
    }
    ...
    function activate() public {
        ...
        ERC721(ve).transferFrom(msg.sender, address(this), id);
    ...
    function startAuction(uint256 amount) external {
    ...
    function bid(uint256 amount) external {
        ...
        require(amount * 1000 >= currentPrice * minIncreaseMultiplier, "bid too low");
        ...
        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance >= amount, "Check the token allowance");
        IERC20(token).transferFrom(address(this), winning, currentPrice);
        IERC20(token).transferFrom(msg.sender, address(this), amount);
    ...
    function endAuction() external {
        ...
        IERC20(token).approve(stakingPool, currentPrice);
    ...
    function buyout() external {
        ...
        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance >= totalBuyoutCost, "not enough to complete buyout");
        IERC20(token).transferFrom(msg.sender, address(this), totalBuyoutCost);
        _burn(msg.sender, balanceOf(msg.sender));

        finalBuyoutPricePerToken = pricePerToken;
        
        // transfer erc721 to redeemer
        IERC721(ve).transferFrom(address(this), msg.sender, id);
    ...
    function redeemShardsForToken() external {
        ...
        transferFrom(address(this), msg.sender, paymentDue);
    }
